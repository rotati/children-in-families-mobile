(function(){
  ...
  CaseNote.$inject = ['$q', 'CaseNoteDomainGroup', '$rootScope'];

  function CaseNote($q, CaseNoteDomainGroup, $rootScope) {
    var CaseNote = null,
        limit    = 1,
        skip     = 0;

        CaseNote = persistence.define('CaseNote', {
          attendee: 'TEXT',
          meetingDate: 'DATE',
          createdAt: 'DATE',
          updatedAt: 'DATE'
    ...
    return {
      ...
      save: _save,
      total: _total,
      caseNoteDomainGroups: _caseNoteDomainGroups,
    ...
    function _save(caseNote) {
      ...
      angular.forEach(caseNote.noteDomainGroups, function(noteDomainGroup) {
        var noteDomainGroupObj = new CaseNoteDomainGroup.entity(noteDomainGroup);
    ...
    function _caseNoteDomainGroups(caseNote) {
      var defer = $q.defer();

      caseNote.caseNoteDomainGroups.list(function(response) {
        defer.resolve(response);
      })
    ...
    function _saveToServer(caseId, data) {
      ...
      var req = {
            method: 'POST',
            url: API.host + '/cases/'+ caseId +'/case_notes',
            data: data
          }
